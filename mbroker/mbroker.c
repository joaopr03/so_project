...
int init_server() {
    ...
    for (int i = 0; i < max_sessions; i++) {
        ...
        strcpy(boxes[i].name, "");
        boxes[i].size = 0;
        if (pthread_mutex_init(&boxes[i].lock, NULL) != 0) {
            return -1;
        }
    }
...
int start_publisher() {
    ...
    while (true) {  
        ...
        if (bytes_read > 0) {
            if (pthread_mutex_lock(&boxes[box_index].lock) != 0) {
                exit(EXIT_FAILURE);
            }
            int fhandle;
            if ((fhandle = tfs_open(box_name, TFS_O_APPEND)) < 0) {
                ...
                boxes[box_index].n_publishers--;
                pthread_mutex_unlock(&boxes[box_index].lock);
                unlink(client_named_pipe_path);
                fprintf(stdout, "Sucessfully ended publisher: box removed\n");
                return EXIT_FAILURE;
            }
            ...
            if (bytes_written <= 0) {
                fprintf(stdout, "ERROR %s\n", "Failed to write box");
                tfs_close(fhandle);
                if (pthread_mutex_unlock(&boxes[box_index].lock) != 0) {
                    exit(EXIT_FAILURE);
                }
                return EXIT_FAILURE;
            }
...
int start_subscriber() {
    ...
    while (true) {
        ...
        int fhandle;
        if ((fhandle = tfs_open(box_name, 0)) < 0) {
            if (close(named_pipe) < 0) {
                fprintf(stdout, "ERROR %s\n", "Failed to close pipe");
                return EXIT_FAILURE;
            }
            ...
            unlink(client_named_pipe_path);
            pthread_mutex_unlock(&boxes[box_index].lock);
            fprintf(stdout, "Sucessfully ended subscriber: box removed\n\n");
            return EXIT_FAILURE;
        }
        ...
        while (true) {
            ...
            if (bytes_processed <= count) {
                if (aux_char == '\0' && bytes_processed < count) {
                    buffer[i++] = aux_char;
                    break;
                } else if (aux_char != '\0') {
                    buffer[i++] = aux_char;
    ...
    boxes[box_index].n_subscribers--;
    fprintf(stdout, "Sucessfully ended subscriber\n");
    return 0;
}
