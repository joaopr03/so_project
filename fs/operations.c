...
int tfs_init(tfs_params const *params_ptr) {
    tfs_params params;
    pthread_mutex_lock(&trinco_william_caralho[0]);  // esse é o init
    if (params_ptr != NULL) {
        params = *params_ptr;
    } else {
...
int tfs_destroy() {
    pthread_mutex_lock(&trinco_william_caralho[1]); // esse é o do destroy / frees
    if (state_destroy() != 0) {
        return -1;
    }
...
int tfs_link(char const *target, char const *link_name) {
    ...
    int inum;
    if ((inum = find_hard_link(root_dir_inode, target + 1)) < 0)
        return -1;
