...
int inode_create(inode_type i_type) {
    ...
    case T_FILE:
        ...
        inode_table[inumber].i_nodes_number = 0;
        break;
    default:
...
void inode_delete(int inumber) {
    ...
    if (inode_table[inumber].i_nodes_number > 0)
        return;
...
int add_sym_link(inode_t *inode, char const *sub_name, char const *target) {
    if (strlen(sub_name) == 0 || strlen(sub_name) > MAX_FILE_NAME - 1) {
        return -1; // invalid sub_name
    }
    ...
    for (size_t i = 0; i < MAX_DIR_ENTRIES; i++) {
        if (dir_entry[i].d_inumber == -1) {
            dir_entry[i].d_inumber = -2;
            strncpy(dir_entry[i].d_name, sub_name, MAX_FILE_NAME - 1);
            dir_entry[i].d_name[MAX_FILE_NAME - 1] = '\0';
            
            strncpy(dir_entry[i].d_link_name, target, MAX_FILE_NAME - 1);
            dir_entry[i].d_name[MAX_FILE_NAME - 1] = '\0';
...
int find_in_dir(inode_t const *inode, char const *sub_name) {
    pthread_mutex_lock(&trinco_william_caralho[13]); // esse é se for possivel eliminar diretorias
    ...
    for (int i = 0; i < MAX_DIR_ENTRIES; i++) {
        pthread_mutex_lock(&trinco_william_caralho[2]); // esse o link and unlink, mexer na memoria
        if ((dir_entry[i].d_inumber != -1) &&
            (strncmp(dir_entry[i].d_name, sub_name, MAX_FILE_NAME) == 0)) {

            int sub_inumber;
            if ((sub_inumber = dir_entry[i].d_inumber) == -2) {
                sub_inumber = find_in_dir(inode, dir_entry[i].d_link_name + 1);
            }
            pthread_mutex_unlock(&trinco_william_caralho[1]);
            pthread_mutex_unlock(&trinco_william_caralho[2]);
            pthread_mutex_unlock(&trinco_william_caralho[13]);
            return sub_inumber;
        }
        pthread_mutex_unlock(&trinco_william_caralho[2]);
    }
